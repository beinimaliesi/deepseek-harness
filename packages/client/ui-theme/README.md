...
`design-platform.css` also owns the product's island palette and icon-tile roles. Light mode uses cream surfaces, warm brown text, mint actions, and a CSS-only organic background pattern; dark mode maps those roles onto forest-green surfaces. Feature packages consume the named tokens and keep code, terminal, and diff typography unchanged. The implementation contains no Animal Island UI source, dependency, font, illustration, or other asset; provenance and the licensing limit are recorded in the [island visual-language decision](../../../.agents/notes/implemented/process/2026-08-14-island-web-visual-language.md).
...
- **The token sheets are the sole color authority** — feature CSS does not carry palette literals. A shared product color enters `design-platform.css` as a semantic alias or a named `--dsw-specific-*` role before components consume it.
