...
`design-platform.css` 还拥有产品的岛屿色板与图标色块角色。浅色主题使用奶油色表面、暖棕文字、薄荷绿操作和纯 CSS 有机背景纹理；深色主题将这些角色映射到森林绿表面。功能包使用具名 token，并保持代码、终端与 diff 的排版不变。实现中不含 Animal Island UI 的源码、依赖、字体、插画或其他资产；来源说明与许可限制记录在[岛屿视觉语言决策](../../../.agents/notes/implemented/process/2026-08-14-island-web-visual-language.md)中。
...
- **token 样式表是颜色值的唯一权威来源**：功能 CSS 不携带色板字面量。共享产品颜色必须先以语义别名或具名 `--dsw-specific-*` 角色进入 `design-platform.css`，组件才能使用。
